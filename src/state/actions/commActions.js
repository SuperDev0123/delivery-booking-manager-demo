...
export function successDeleteComm(data) {
    return {
        type: SUCCESS_DELETE_COMM,
        comm: data,
    };
}

export function failedDeleteComm(error) {
    return {
        type: FAILED_DELETE_COMM,
        errorMessage: 'Unable to delete Comm. Error:' + error,
    };
}
...
export function setLocalFilter(key, value) {
    ...
    } else if (key === 'sortByDate') {
        return {
            type: SET_LOCAL_FILTER_SORTBYDATE,
            sortByDate: value,
        };
    } else if (key === 'dropdownFilter') {
        return {
            type: SET_LOCAL_FILTER_DROPDOWN,
            dropdownFilter: value,
        };
    }
}

export function setAllLocalFilter(bookingId, sortField, sortType, columnFilters, simpleSearchKeyword, sortByDate=false, dropdownFilter='All') {
    return {
